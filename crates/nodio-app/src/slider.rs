#![allow(dead_code)]
///! Modified version of [egui::widgets::Slider] for audio channel visualization
use eframe::egui;
use egui::*;
...
/// Specifies the orientation of a [`VolumeSlider`].
...
impl<'a> VolumeSlider<'a> {
    pub fn new(value: &'a mut f32, peak_values: (f32, f32)) -> Self {
        let range = 0.0..=100.0;
...
impl<'a> VolumeSlider<'a> {
    ...
    fn slider_ui(&mut self, ui: &mut Ui, response: &Response) {
        ...
        if ui.is_rect_visible(response.rect) {
            ...
            let position_peak_left =
                self.position_from_value(self.peak_values.0 as f64 * value, position_range.clone());
            let position_peak_right =
                self.position_from_value(self.peak_values.1 as f64 * value, position_range);

            let mut peak_left_rect = self.rail_rect(rect, rail_radius);
            let mut peak_right_rect = self.rail_rect(rect, rail_radius);

            match self.orientation {
                SliderOrientation::Horizontal => {
                    peak_left_rect.max.x = position_peak_left;
                    peak_right_rect.max.x = position_peak_right;
                    peak_left_rect.max.y -= rail_radius;
                    peak_right_rect.min.y += rail_radius;
                }
                SliderOrientation::Vertical => {
                    peak_left_rect.max.y = position_peak_left;
                    peak_right_rect.max.y = position_peak_right;
                    peak_left_rect.max.x -= rail_radius;
    ...
    fn position_range(&self, rect: &Rect) -> RangeInclusive<f32> {
        let handle_radius = 0.0; //self.handle_radius(rect);
        match self.orientation {
            SliderOrientation::Horizontal => {
    ...
    fn handle_rect(&self, center: Pos2, rect: &Rect) -> Rect {
        let size = match self.orientation {
            SliderOrientation::Horizontal => Vec2::new(8.0, rect.height() / 1.25),
            SliderOrientation::Vertical => Vec2::new(rect.width() / 1.25, 8.0),
        };
    ...
    fn value_ui(&mut self, ui: &mut Ui, position_range: RangeInclusive<f32>) -> Response {
        let speed = self.current_gradient(&position_range);
...
impl<'a> Widget for VolumeSlider<'a> {
