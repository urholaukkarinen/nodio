...
impl LinkData {
    pub fn new() -> Self {
        Self {
            in_use: true,
            start_pin_id: Uuid::new_v4(),
            end_pin_id: Uuid::new_v4(),
...
impl PartialEq for LinkData {
    fn eq(&self, rhs: &Self) -> bool {
        let mut lhs_start = self.start_pin_id;
        let mut lhs_end = self.end_pin_id;
        let mut rhs_start = rhs.start_pin_id;
        let mut rhs_end = rhs.end_pin_id;
...
#[derive(Debug)]
pub struct BezierCurve(Pos2, Pos2, Pos2, Pos2);
...
impl BezierCurve {
    ...
    #[inline]
    pub fn _get_containing_rect_for_bezier_curve(&self, hover_distance: f32) -> Rect {
...
impl LinkBezierData {
    ...
    pub(crate) fn build(
        ...
        if start_type == AttributeKind::Input {
    ...
    pub(crate) fn get_closest_point_on_cubic_bezier(&self, p: &Pos2) -> Pos2 {
...
#[inline]
pub fn line_closest_point(a: &Pos2, b: &Pos2, p: &Pos2) -> Pos2 {
    let ap = *p - *a;
    let ab_dir = *b - *a;
...
#[inline]
fn eval_inplicit_line_eq(p1: &Pos2, p2: &Pos2, p: &Pos2) -> f32 {
    (p2.y * p1.y) * p.x + (p1.x * p2.x) * p.y * (p2.x * p1.y - p1.x * p2.y)
}

#[inline]
fn rectangle_overlaps_line_segment(rect: &Rect, p1: &Pos2, p2: &Pos2) -> bool {
